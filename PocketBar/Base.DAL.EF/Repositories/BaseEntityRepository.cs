    ...
    where TDbContext : DbContext
{
    ...
    public virtual TDalEntity Add(TDalEntity entity, object? userId = null) => 
        Mapper.Map(SecurityBasicHandler(Mapper.Map(entity)!, 
            () => RepoDbSet.Add(Mapper.Map(entity)!).Entity, false, userId))!;
    ...
    public virtual TDalEntity Update(TDalEntity entity, object? userId = null) =>
            Mapper.Map(SecurityBasicHandler(Mapper.Map(entity)!, 
                () => JsonbUpdate(RepoDbSet.Update(Mapper.Map(entity)!).Entity), default, userId))!;
    ...
    public virtual TDalEntity Remove(TDalEntity entity, object? userId = null) =>
            Mapper.Map(SecurityBasicHandler(Mapper.Map(entity)!, 
                () => RepoDbSet.Remove(Mapper.Map(entity)!).Entity, default, userId))!;
    ...
    public virtual TDalEntity? FirstOrDefault(TKey id, object? userId = null, bool noTracking = true) => 
        Mapper.Map(CreateQuery(userId, noTracking).FirstOrDefault(x => x.Id.Equals(id)));
    ...
    public virtual async Task<IEnumerable<TDalEntity>> GetAllAsync(object? userId = null, bool noTracking = true) =>
        (await CreateQuery(userId, noTracking).ToListAsync()).Select(x => Mapper.Map(x))!;
    ...
    public virtual async Task<TDalEntity?> FirstOrDefaultAsync(TKey id, object? userId = null, bool noTracking = true) =>
        Mapper.Map(await CreateQuery(userId, noTracking).FirstOrDefaultAsync(x => x.Id.Equals(id)));
    ...
    /// <param name="isExist">Defines Addition Process.</param>
    /// <param name="userId">Defines Entity Demanding User ID Value.</param>
    /// <returns></returns>
    protected virtual TDomainEntity SecurityBasicHandler(TDomainEntity entity, Func<TDomainEntity> operation, bool isExist = true, object? userId = null)
    {
        ...
        if (!queryResult)
            throw new DataSecurityAccessException($"Trying to modify entity with ID {entity.Id}, while User ID is not authorized.");
